...
parser.add_argument('--model_arch', type=str, default='convnext_tiny', choices=['vit_moe', 'resnet50', 'resnet101', 'convnext_tiny', 'efficientnet_b0', 'convnext_small', 'convnext_base'], help='Model architecture to use')
...
def create_model(model_arch, config):
    if model_arch == 'vit_moe':
        ...
        return ModelWrapper(model, config.num_class)
    elif model_arch == 'convnext_base':
        model = timm.create_model('convnext_base', pretrained=True, num_classes=config.num_class)
        return ModelWrapper(model, config.num_class)
