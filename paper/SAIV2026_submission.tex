...
\end{equation}
RT adds an adversarial term using PGD-generated perturbations $x^{\text{adv}} = \arg\max_{x' \in B_\varepsilon(x)} \text{CE}(\mathcal{G}(x'), m)$ with $\varepsilon = 8/255$:
\begin{equation}
...
Standard $\alpha$--$\beta$-CROWN verifies monolithic networks. Verifying our full \textsf{MetaMoE} system (router + 2 experts, ${\sim}$231K parameters) monolithically is \emph{structurally infeasible} with current tools for two reasons. First, ONNX export is trace-based~\cite{pytorchonnx}: it records a single execution path, hard-coding the router's expert selection into a static graph~\cite{pytorchcontrolflow}. The ONNX IR is a static computation graph whose \texttt{If} operator requires identically shaped branch outputs~\cite{onnxir,pytorchcontrolflow}, incompatible with experts having different class space dimensions. Second, all current verifiers -- bound propagation (CROWN~\cite{xu2020abcrown}, $\alpha$--$\beta$-CROWN~\cite{wang2021}), SMT-based (Marabou~\cite{katz2022marabou}), and abstract-interpretation (ERAN~\cite{singh2019eran}) -- require fixed computational graphs; no VNN-COMP benchmark (2021--2025) has included input-dependent routing~\cite{kaulen2025}. Our compositional decomposition sidesteps both barriers by reducing the problem to standard feedforward CNN verification. This motivates a \textbf{compositional verification strategy}:
...
\textbf{Experiments.} We run four groups: (1)~router verification at $\varepsilon \in \{2/255, 4/255, 8/255\}$ under NRT and RT (40 samples); (2)~expert verification across 4 variants and 3 $\varepsilon$ budgets (20 samples each); (3)~empirical PGD robustness ($\varepsilon = 8/255$) for all 8 MoE configurations (40 total runs); and (4)~AA vs.\ ExCRA ordering comparison.
