...
\begin{abstract}
Mixture-of-experts (MoE) architectures offer modularity and scalability, yet their robustness, and the practicality of certifying that robustness, in heterogeneous settings are not well understood. This work presents \textsf{MetaMoE}, a heterogeneous MoE framework designed for compositional formal verifiability. In \textsf{MetaMoE}, a lightweight neural network, called router, takes an input image, classifies its domain, and routes it to the corresponding domain expert neural network for fine-grained classification; we study how robustness propagates compositionally across these router and domain experts and establish when system-level verification can be derived from component-level verification. In homogeneous MoE, all experts share the same task, so a misroute may still yield a correct prediction if the receiving expert classifies the input correctly; in heterogeneous MoE, experts operate on disjoint class spaces across distinct domains, making any misroute catastrophic. We prove that when the router maintains expert selection within a perturbation limit, and the selected expert also maintains its classification within that perturbation limit, the end-to-end system robustness is composable, enabling modular and scalable verification (the computational power needed for verification and re-verification does not snowball as the system expands). We further show that formal certified robustness accuracy (higher computational cost) correlates with empirical accuracy (lower computational cost) which shows the number of perturbed inputs correctly classified. Experiments on heterogeneous datasets demonstrate that robust experts improve adversarial accuracy by up to $13.1\%$ and enable higher formal certifiability, while verified routers achieve $100\%$ certified routing accuracy at practical perturbation for dissimilar datasets. Together, these results show empirical robustness testing and formal certification reinforce each other in heterogeneous MoE systems, and outline a principled path toward scalable, verifiable modular AI. 
...
We also examine the case where experts handle visually similar datasets: specifically, GTSRB~\cite{gtsrb} and PTSD~\cite{ptsd} (both traffic sign recognition tasks with 43 classes each), where the disjoint-class assumption still holds but the router faces a harder discrimination problem. We report these results in Appendix~\ref{app:similarity} \BW{By my count you have 2 more pages, SAIV is 18 page limit not including references/appendices. Try and include all in the main body, particularly results.}.

\subsection{Empirical--Formal Gap}

Two complementary metrics quantify robustness: \emph{adversarial accuracy} $\text{AA}(\varepsilon)$, the fraction of test samples correctly classified under a PGD attack with perturbation budget~$\varepsilon$, and \emph{certified robustness accuracy} $\text{CRA}(\varepsilon)$, the fraction for which a formal verifier proves robustness within the $\varepsilon$-ball. We define the \emph{certification gap} as the difference between these two quantities~\eqref{eq:certification_gap}:
\begin{equation}
...
Inference latency follows $L(k) = L_{\text{router}} + \sum_{i=1}^k L_{\text{expert}_i}$, where $L_{\text{router}}$ is constant and each expert contributes $\sim$40--50ms. Figure~\ref{tab:inference} confirms $O(k)$ scaling: at fixed $k{=}1$, latency remains $\sim$71ms regardless of whether the system contains 2 or 5 total experts (H5).
