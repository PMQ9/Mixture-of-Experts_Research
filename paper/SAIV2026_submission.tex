...
\begin{axis}[
    ...
    ymin=0, ymax=118,
    ylabel={(\%)},
    symbolic x coords={C10 NRT, C10 RT, MNIST NRT, MNIST RT, Rtr NRT, Rtr RT},
    xtick=data,
    ...
    every axis plot/.append style={fill opacity=0.85},
    clip=false,
    % Separator between experts and router
    extra x ticks={},
]
...
\addplot[fill=green!50, draw=green!70,
    ...
    (C10 NRT, 0.0) +- (0,0.0)
    (C10 RT, 90.0) +- (0,0.0)
    (MNIST NRT, 75.0) +- (0,0.0)
    (MNIST RT, 100.0) +- (0,0.0)
    (Rtr NRT, 97.5) +- (0,0.0)
    (Rtr RT, 100.0) +- (0,0.0)
};
% Gap annotations (Delta) above bar pairs
\node[font=\scriptsize, text=black!70] at (axis cs:C10 NRT, 8) {--};
\node[font=\scriptsize, text=black!70] at (axis cs:C10 RT, 98) {$\Delta{=}73$};
\node[font=\scriptsize, text=black!70] at (axis cs:MNIST NRT, 83) {$\Delta{=}29$};
\node[font=\scriptsize, text=black!70] at (axis cs:MNIST RT, 108) {$\Delta{=}9$};
\node[font=\scriptsize, text=black!70] at (axis cs:Rtr NRT, 106) {$\Delta{=}62$};
\node[font=\scriptsize, text=black!70] at (axis cs:Rtr RT, 108) {$\Delta{=}65$};
% Separator line between experts and router
\draw[thin, dashed, gray!60] (axis cs:MNIST RT, 0) ++(0.55cm,0) -- ++(0,5.6cm);
\legend{Adversarial Accuracy (AA), Certified Robustness (CRA)}
\end{axis}
% Component group labels
\node[font=\scriptsize, text=gray] at (2.6,-0.55) {Experts};
\node[font=\scriptsize, text=gray] at (5.85,-0.55) {Router};
\end{tikzpicture}
