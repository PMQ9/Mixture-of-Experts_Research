...
The central question of this paper is whether a modular MoE system can be verified \emph{component-by-component} rather than as a monolithic whole. In assume-guarantee reasoning~\cite{henzinger1998}, a system property is decomposed into obligations on individual components: each component \emph{assumes} that its environment satisfies certain conditions and \emph{guarantees} a local property, and the composition of these local guarantees implies the system-level property. We apply this principle to hard-routed MoE: the router assumes a well-formed input and guarantees stable expert selection; each expert assumes it receives inputs from its own domain and guarantees correct classification. The theorem below formalizes this decomposition, proving that these two component-level guarantees are individually necessary and jointly sufficient for system robustness -- that is, no interaction effects between router and expert can invalidate component-level certificates. We proceed with a definition, two lemmas, and the composing theorem.
...
The practical consequence of this assume-guarantee decomposition is \emph{modular verification}: verify router invariance and expert robustness independently, then compose guarantees via Theorem~\ref{thm:compositional}. This avoids verifying the full MoE as a monolithic network, reducing verification cost and enabling incremental certification -- when a new expert is added, only the new expert and the updated router require (re-)verification, while existing expert certificates remain valid.
...
Figure~\ref{fig:gap-analysis} compares expert AA and CRA at $\varepsilon = 2/255$, where both metrics measure classification correctness on the same samples. Within each domain, higher AA monotonically associates with higher CRA: MNIST NRT$\to$RT increases both AA ($46\% \to 91\%$) and CRA ($75\% \to 100\%$); CIFAR-10 NRT$\to$RT moves from completely unverifiable (CRA $= 0\%$) to CRA $= 90\%$. CRA can exceed AA because the verifier proves correctness under \emph{all} perturbations in the $\varepsilon$-ball, whereas PGD (7 iterations) may not find the worst-case example. The resulting gap $\Delta = \text{CRA} - \text{AA}$ is domain-dependent: $\Delta = 9\%$ for MNIST-RT (PGD nearly exhausts the perturbation space) vs.\ $\Delta = 73\%$ for CIFAR-10-RT (looser CROWN bounds and weaker PGD convergence on complex decision boundaries). We treat $\Delta$ as a diagnostic of bound tightness relative to attack strength, not as a universal robustness surplus. Routers are excluded from this comparison because router CRA verifies routing invariance (a per-sample argmax property) while router AA measures end-to-end classification, making their difference uninformative (see Section~\ref{sec:eval-formal}).
...
\end{axis}
\end{tikzpicture}
\caption{Expert empirical--formal robustness comparison at $\varepsilon = 2/255$ (mean $\pm$ std over 5 runs). $\Delta = \text{CRA} - \text{AA}$ annotated above each pair. Within each domain, higher AA associates with higher CRA, but the absolute gap is domain-dependent: MNIST-RT achieves tight correspondence ($\Delta = 9\%$), while CIFAR-10-RT shows a large gap ($\Delta = 73\%$) reflecting looser CROWN bounds on complex domains. CIFAR-10 NRT is completely unverifiable (both metrics $\approx 0\%$). Router results are omitted because router CRA (routing invariance) and router AA (end-to-end classification) measure different properties; see Section~\ref{sec:eval-formal}.}
\label{fig:gap-analysis}
\end{figure}

The practical implication is clear: robust training of experts is mandatory for verifiability. CIFAR-10-NRT is completely unverifiable (CRA $= 0\%$), while within RT models, higher empirical robustness correlates with higher formal certification in our study.
...
Compositional verification is not merely more efficient than monolithic MoE verification -- it is currently the \emph{only} viable approach due to two independent tool limitations in the verification pipeline.

\emph{Limitation 1: ONNX cannot faithfully represent dynamic MoE routing.}
Standard model export via \texttt{torch.onnx.export} is trace-based: the exporter executes the model once on a dummy input and records the single execution path as a static computational graph~\cite{pytorchonnx}. In MoE forward passes, the router's \texttt{argmax} or \texttt{top\_k} output determines which experts are activated -- a data-dependent control flow decision. The tracer evaluates this decision for the specific dummy input and \emph{hard-codes} the resulting expert selection into the exported graph~\cite{pytorchcontrolflow}. For example, if the dummy input routes to expert~0, the exported ONNX model always routes to expert~0, regardless of future inputs. The ONNX intermediate representation is fundamentally a static, topologically ordered, side-effect-free computation graph~\cite{onnxir} and cannot natively represent input-dependent conditional execution. While ONNX defines an \texttt{If} operator, using it requires rewriting the model to use \texttt{torch.cond()} and both branches must produce identically shaped outputs~\cite{pytorchcontrolflow} -- constraints incompatible with MoE routing, where different experts may have different class space dimensions.

\emph{Limitation 2: Neural network verifiers cannot reason about conditional execution.}
Even if ONNX could represent dynamic routing, current verifiers cannot verify such models. Bound propagation methods (CROWN~\cite{xu2020abcrown}, $\alpha$--$\beta$-CROWN~\cite{wang2021}) compute linear relaxations layer by layer through a \emph{fixed} computational graph; if the graph topology varies per input, bound propagation is undefined. Concretely, auto\_LiRPA -- the bound propagation engine underlying $\alpha$--$\beta$-CROWN -- captures the model via \texttt{torch.jit.trace}, which eliminates all control flow, and its operator registry implements no control-flow operators (\texttt{If}, \texttt{Loop}, etc.); unrecognized operators raise \texttt{NotImplementedError}~\cite{xu2020abcrown}. This is not specific to $\alpha$--$\beta$-CROWN: SMT-based verifiers such as Marabou~\cite{katz2022marabou} and abstract-interpretation verifiers such as ERAN~\cite{singh2019eran} likewise operate on fixed network topologies. No VNN-COMP benchmark (2021--2025) has included models with input-dependent routing or conditional execution~\cite{kaulen2025}, reflecting a community-wide gap.

These two limitations make monolithic MoE verification structurally infeasible with current tools. Compositional verification sidesteps both by decomposing the system into static components -- a standalone router and independent experts -- each of which is a standard feedforward CNN amenable to existing export and verification pipelines. This architectural decomposition, justified by Theorem~\ref{thm:compositional}, transforms a structurally unsupported problem into multiple standard problems, yielding not only the $O(1)$ per-component verification cost but also compatibility with all existing and future monolithic verifiers.
...
Our experiments use small verification-compatible CNNs (${\sim}$96K parameters per expert, ${\sim}$38K for the router). This scale is representative of embedded safety-critical deployments such as traffic sign classifiers on edge devices, but production vision systems often employ ResNets or Vision Transformers with millions of parameters. The compositional decomposition reduces the verification target, yet verifying a single large expert remains an open challenge for the verification community -- $\alpha$--$\beta$-CROWN and comparable tools face memory and time constraints well before reaching that scale.

The framework also depends on the router's ability to discriminate between expert domains. For visually dissimilar datasets (CIFAR-10 vs.\ MNIST), the router achieves 100\% gating accuracy and Theorem~\ref{thm:compositional} applies directly. For visually similar datasets (GTSRB vs.\ PTSD, both traffic sign recognition), gating accuracy drops to 93.17\% (Appendix~\ref{app:similarity}), violating router invariance at ${\sim}$7\% of inputs. Systems with many similar-domain experts would face compounding routing errors.

We verified 20--40 samples per configuration at a cost of ${\sim}$11 seconds per router sample and up to 300 seconds per expert sample. While zero falsifications across all configurations is encouraging, a larger verification budget would strengthen statistical confidence. We also consider only $\ell_\infty$-bounded perturbations; other threat models ($\ell_2$, patch attacks, common corruptions) may exhibit different robustness characteristics and require separate verification specifications. Finally, the certification gap $\Delta$ between CRA and AA varies from $9\%$ (MNIST-RT) to $73\%$ (CIFAR-10-RT) at $\varepsilon = 2/255$, indicating that empirical robustness is a necessary but not sufficient proxy for formal certifiability on complex domains where CROWN bounds remain loose.
...
The experiments reveal that for dissimilar datasets, expert robustness dominates system behavior (13.15\% adversarial accuracy difference between all-RT and all-NRT experts) while the router training paradigm has negligible impact ($\Delta < 0.1\%$), allowing a 3.2$\times$ router training speedup by forgoing adversarial augmentation. Router verification achieves 100\% certified routing accuracy at $\varepsilon \leq 4/255$ with zero falsifications across all 40 runs, satisfying the invariance condition of Theorem~\ref{thm:compositional}. Within each domain, higher empirical adversarial accuracy monotonically associates with higher certified robustness, and non-robust training on complex domains yields complete verification failure (CRA $= 0\%$ for CIFAR-10-NRT), establishing robust training as a prerequisite for verifiability.

We see three directions for extending this work. First, generalizing Theorem~\ref{thm:compositional} to soft and top-$k$ routing will require reasoning about weight stability under perturbation; probabilistic verification methods~\cite{chwialkowski2025probabilistic} may offer relaxed but practical guarantees in that setting. Second, closing the empirical--formal gap on complex domains calls for tighter bound propagation or certified training methods such as CROWN-IBP~\cite{zhang2020} integrated into the expert training pipeline. Third, the GTSRB+PTSD results (Appendix~\ref{app:similarity}) expose a richer problem than routing failure alone. When experts cover visually similar domains -- both datasets contain 43 traffic sign classes, many of which are semantically shared (e.g., stop signs, speed limits) -- a misroute does not necessarily cause a misclassification: the receiving expert may still classify the input correctly if the relevant class appears in its own label space. Under our current formulation, any misroute violates router invariance and the biconditional of Theorem~\ref{thm:compositional} treats it as a system failure, yet the end-to-end prediction may remain correct. Relaxing the theorem to account for overlapping or semantically equivalent classes across experts would yield a weaker but more permissive compositional guarantee, where verification need only confirm that \emph{some} reachable expert classifies correctly rather than that the \emph{intended} expert is always selected. Formalizing this relaxation, and designing routers that exploit cross-expert redundancy rather than treating every misroute as catastrophic, is a natural next step for scaling the framework to systems with many related-domain experts. We hope the compositional perspective introduced here provides a useful foundation for building verifiable modular AI in safety-critical applications.
