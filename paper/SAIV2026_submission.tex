...
This paper introduces \textsf{MetaMoE} (Figure~\ref{fig:metamoe-arch}), a verification-aware heterogeneous MoE architecture in which each input carries two labels: a fine-grained \emph{class} (\emph{e.g.}, speed-limit sign) and a coarse \emph{meta-class} identifying the domain (\emph{e.g.}, German traffic signs vs.\ handwritten digits). The router learns to predict the meta-class and dispatches the input to the corresponding domain expert, which then predicts the fine-grained class. The framework combines verifiable experts with a verifiable routing network, intermixing adversarially and non-adversarially trained components, and is complemented by empirical adversarial experiments under Projected Gradient Descent (PGD) attacks~\cite{madry2018} that test whether the same robustness trends hold across model variants. Under hard routing, exactly one expert processes each input, so the system decomposes naturally into two independent verification tasks: does the router select the right expert?, and does that expert classify correctly? If both hold, the system is correct. This decomposition leads to three contributions, each building on the previous:

\begin{figure}[t]
% alt={Block diagram of the MetaMoE architecture. An input image x feeds into a Meta Gating Network G that outputs expert probabilities. These probabilities select among parallel domain experts E_0, E_1, ..., E_N; the chosen expert's output populates a segment of a unified combined output vector y-hat. To the right of the input, dual labels are annotated: a coarse meta-class m predicted by the router and a fine-grained domain class y predicted by the selected expert.}
\centering
...
\end{equation*}
representing its contribution to the final output. To combine outputs from experts with different class spaces, we define \emph{class offsets} $o_i = \sum_{m<i} C_m$ so that expert~$i$'s classes occupy positions $o_i$ through $o_i + C_i - 1$ in a unified output vector $\hat{y} \in \mathbb{R}^{C_{\text{total}}}$. For each output index $c$, let $\mathcal{A}(c) = \{\, j \in \{1,\ldots,k\} : c \in \{o_{s_j}, \ldots, o_{s_j} + C_{s_j} - 1\}\,\}$ denote the subset of activated experts whose class range contains~$c$. The unified MoE output $\hat{y}$ (distinct from the target label~$y$) is:
\begin{equation}
\label{eq:first}
    \hat{y}_c = \begin{cases}
        \displaystyle \sum_{j \in \mathcal{A}(c)} \frac{w_j}{\sum_{\ell \in \mathcal{A}(c)} w_\ell}\; y_{s_j}[c - o_{s_j}], & \text{if } \mathcal{A}(c) \neq \emptyset \\[4pt]
        0, & \text{otherwise}
    \end{cases}
\end{equation}
where $y_{s_j}[c - o_{s_j}]$ is expert~$s_j$'s logit at local class index $c - o_{s_j}$, and the per-index renormalization makes contributions a convex combination of expert logits when multiple activated experts share that class. In the disjoint-class hard-routing regime verified here, $|\mathcal{A}(c)| \leq 1$ and \eqref{eq:first} collapses to $\hat{y}_c = y_{s_{j^*}}[c - o_{s_{j^*}}]$. Proving the general form for the soft and top-$k$ extensions is left as future work, see Section~\ref{sec:limitations}.

\subsection{Adversarial Threat Model and Robustness}
\label{sec:threat-model}

We consider an adversary that applies a small perturbation $\delta \in \mathbb{R}^{C \times H \times W}$ to the input, bounded under the $\ell_\infty$ norm: $\|\delta\|_\infty \leq \varepsilon$, where $\varepsilon > 0$ is the perturbation budget (\emph{e.g.}, $\varepsilon = 8/255 \approx 0.031$ in pixel intensity). This produces adversarial examples $x' = x + \delta$ lying within the $\ell_\infty$ ball $B_\varepsilon(x) = \{x' : \|x' - x\|_\infty \leq \varepsilon\}$, i.e., all images within $\varepsilon$ of $x$ in every pixel channel. We keep the verification-literature term ``$\ell_\infty$ ball,'' but $B_\varepsilon(x)$ is geometrically an axis-aligned hypercube of side $2\varepsilon$; our certified guarantees hold over this cube and not over any wider neighborhood of~$x$. Robustness requires that the model's prediction remains correct for \emph{all} perturbations within this ball:
...
\textbf{Relation to constraint-based metrics.} Casadio et al.~\cite{casadio2022} cast robustness as a verification property with three metrics: \emph{constraint satisfaction} (per-input verdict), \emph{constraint accuracy} (fraction satisfying it on the clean point alone), and \emph{constraint security} (fraction for which it holds throughout the perturbation region). In their terms, our $\mathbbm{1}[\text{Verify}(\cdot)]$ is a constraint-satisfaction verdict, ExCRA and RoCRA are constraint-security aggregates over expert correctness and router invariance, and clean accuracy is constraint accuracy. Theorem~\ref{thm:compositional} then reads as composing two constraint-security guarantees into a system-level one.
...
\begin{figure}
% alt={Three-panel illustration of compositional robustness. Panel (a) shows the clean input x at the center of a perturbation ball B_epsilon(x) containing scattered perturbed points x'. Panel (b) shows router invariance: all perturbed points in the ball route to the same selected expert E_{m*}, while other experts are rejected. Panel (c) shows expert robustness: E_{m*} predicts the correct class y for every point in the ball. A box below states Theorem 1: the MoE is robust at x if and only if both (b) and (c) hold.}
\centering
...
\textbf{Why heterogeneous vision MoE?} Deployed vision systems rarely stay within a single training distribution: handling inputs that span disjoint domains is common and dispatching each input to a dataset-specific expert is therefore a natural design. We evaluate on both a visually-dissimilar pair (CIFAR-10 vs.\ MNIST) and a structurally-similar one (GTSRB+PTSD).
...
\begin{figure}[t]
% alt={Stacked bar chart of router formal verification outcomes over 40 samples for six configurations: NRT and RT routers at epsilon equal to 2/255, 4/255, and 8/255. All five configurations except NRT at 8/255 reach 40 verified samples; NRT at 8/255 shows 24 verified and 16 timeouts. No falsifications occur in any configuration.}
\centering
\begin{tikzpicture}
\begin{axis}[
    ybar stacked,
    ...
    legend style={at={(0.5,-0.85)}, anchor=north, font=\small, legend columns=2},
    grid=major,
...
\legend{\vspace{0.2cm}Verified, Timeout (not falsified)}
\end{axis}
\end{tikzpicture}
...
\begin{figure}[!t]
% alt={Four-by-three heatmap of Expert Certified Robustness Accuracy (ExCRA) with rows for CIFAR-10 NRT, CIFAR-10 RT, MNIST NRT, and MNIST RT, and columns for epsilon equal to 2/255, 4/255, and 8/255. Values: CIFAR-10 NRT is 0%, 0%, 0%; CIFAR-10 RT is 90%, 54%, 0%; MNIST NRT is 75%, 0%, 0%; MNIST RT is 100%, 99%, 95%. Colors range from red (0%) through yellow (50%) to green (100%), with a colorbar at the bottom.}
\centering
\begin{tikzpicture}[
...
\begin{figure}[t]
% alt={Grouped bar chart of clean accuracy (blue) versus PGD adversarial accuracy at epsilon=8/255 (red) for four expert configurations. CIFAR-10 NRT: 83.75% clean, 0.01% adversarial. CIFAR-10 RT: 77.54% clean, 17.24% adversarial. MNIST NRT: 99.19% clean, 46.06% adversarial. MNIST RT: 99.17% clean, 90.77% adversarial. Error bars show standard deviation over 5 runs. The gap between clean and adversarial bars (the robustness gap) narrows substantially under RT, dramatically so for MNIST.}
\centering
\begin{tikzpicture}
\begin{axis}[
    ybar,
...
\begin{figure}[t]
% alt={Grouped bar chart comparing empirical Adversarial Accuracy AA at epsilon=8/255 (red) and Expert Certified Robustness Accuracy ExCRA at epsilon=2/255 (green) for CIFAR-10 and MNIST experts under NRT and RT training. CIFAR-10 NRT shows about 0% on both metrics. CIFAR-10 RT shows 17% AA and 90% ExCRA. MNIST NRT shows 46% AA and 75% ExCRA. MNIST RT shows 91% AA and 100% ExCRA. A dashed line separates the two domains. Within each domain, the NRT-to-RT ordering is consistent across both metrics.}
\centering
\begin{tikzpicture}
\begin{axis}[
    ybar,
...
\begin{figure}[t]
% alt={Stacked horizontal bar chart of training time in minutes comparing monolithic and MetaMoE training. For two datasets (GTSRB + CIFAR-10), monolithic uses 91.5 minutes of joint training; MetaMoE uses 87.5 minutes for experts plus 12 minutes for the router, totaling 99.5 minutes. For incremental addition of MNIST, monolithic carries 91.5 minutes of prior training and adds 151 minutes of full retraining (242.5 minutes total); MetaMoE carries 99.5 minutes of prior training, adds 72.5 minutes for the new MNIST expert and 30 minutes of router fine-tuning, totaling 202.0 minutes. MetaMoE's incremental cost is 32% lower than monolithic.}
\centering
...
\begin{figure}[t]
% alt={Line plot of inference latency in milliseconds versus meta_top_k (1 to 5) for systems containing 2, 3, 4, or 5 total experts. At k=1, latency ranges from approximately 71 to 78 ms regardless of expert count. Latency grows roughly linearly with k for each curve: the 2-expert system reaches 79 ms at k=2; the 3-expert system reaches 94 ms at k=3; the 4-expert system reaches 128 ms at k=4; the 5-expert system reaches 156 ms at k=5.}
\centering
\begin{tikzpicture}
\begin{axis}[
    width=0.74\columnwidth,
...
Theorem~\ref{thm:compositional} is established for hard routing ($k{=}1$) with disjoint class spaces. Soft or top-$k$ routing requires invariance of the full top-$k$ set and routing-weight stability; overlapping class spaces weaken the result to a one-sided implication (Section~\ref{sec:compositional}). The framework also depends on router discrimination: CIFAR-10 vs.\ MNIST gives 100\% gating accuracy, while visually similar GTSRB vs.\ PTSD drops to 93.17\%, violating invariance at ${\sim}$7\% of inputs.

Our experiments use small verification-compatible CNNs on 20--40 samples per configuration (${\approx}$11s/router, up to 300s/expert); larger experts or sample budgets remain bounded by current tool capacity. We consider only $\ell_\infty$ perturbations; $\ell_2$ and patch attacks need separate specifications.

\textbf{Scope of the certified guarantee.} Theorem~\ref{thm:compositional} certifies each tested cube $B_\varepsilon(x)$ independently, not the input distribution, this is a \emph{cube-wise} guarantee (Casadio et al.'s constraint security~\cite{casadio2022}) shared by all current complete verifiers~\cite{kaulen2025} and is silent about unseen images. PGD-AA~\cite{madry2018} provides the empirical complement.

\section{Conclusion and Future Work}

We presented \textsf{MetaMoE}, a framework for compositional robustness verification of heterogeneous MoE systems. Theorem~\ref{thm:compositional} reduces system-level verification of hard-routed MoE with disjoint expert class spaces to the largest single component, with the advantage scaling linearly in the number of experts. Combining $\alpha$--$\beta$-CROWN certification with empirical adversarial evaluation, we find that expert robustness dominates system behavior (13.15\% AA gap between all-RT and all-NRT experts) while the router training paradigm is near-irrelevant ($< 0.1\%$), enabling a 3.2$\times$ router training speedup by forgoing adversarial augmentation. Router verification achieves 100\% RoCRA at $\varepsilon \leq 4/255$ across all 40 runs, satisfying the invariance condition of Theorem~\ref{thm:compositional}.

We see four directions for extending this work. First, generalizing Theorem~\ref{thm:compositional} to soft and top-$k$ routing will require reasoning about weight stability under perturbation; probabilistic verification methods~\cite{chwialkowski2025probabilistic} may offer relaxed but practical guarantees in that setting. Second, extending these empirical results to more complex domains will require tighter bound propagation or certified training methods such as CROWN-IBP~\cite{zhang2020} integrated into the expert training pipeline. Third, the GTSRB+PTSD results (93.17\% gating accuracy vs.\ 100\% for CIFAR10+MNIST) expose a richer problem than routing failure alone: when experts share semantically equivalent classes, a misroute may not cause a misclassification. Relaxing Theorem~\ref{thm:compositional} to account for cross-expert redundancy would yield a more permissive compositional guarantee for similar-domain systems. 

Fourth, the same view may scale to MoE LLMs such as Mixtral~\cite{jiang2024}, DeepSeekMoE~\cite{dai2024}, and DeepSeek-V4~\cite{deepseekv4}, which route each token to a sparse subset of hundreds of experts: a port of Theorem~\ref{thm:compositional} would require the soft-routing extension above, distributional rather than argmax specifications~\cite{casadio2022,chwialkowski2025probabilistic}, and per-expert verifiers that scale to transformer blocks~\cite{kaulen2025}.

The compositional perspective introduced here provides a foundation for building verifiable modular AI in safety-critical applications. The code is available: \url{https://github.com/PMQ9/Mixture-of-Experts_Research}.
