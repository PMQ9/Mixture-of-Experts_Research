...
The router $\mathcal{G}(x) \in \mathbb{R}^N$ produces $N$ raw logits and selects expert $m^* = \arg\max_i g_i$. It shares the expert's convolutional backbone but omits BatchNorm (${\sim}$38K params for $N{=}2$), with its final layer outputting $N$ routing logits instead of $C_i$ class logits.
...
Applying $\alpha$--$\beta$-CROWN to our compositional setting requires several adaptations. By Theorem~\ref{thm:compositional}, we export only the router as a standalone ONNX model (${\sim}$38K parameters), a ${\sim}$6$\times$ reduction from the full system. Perturbation bounds are applied in the unified normalization space so that the verified $\varepsilon$ matches the space used during adversarial training.
...
All experiments use VerifiableCNN experts (${\sim}$94--96K parameters) and a VerifiableCNN router without BatchNorm (${\sim}$38K parameters), trained under NRT or RT. Results are averaged over 5 independent runs on an NVIDIA GeForce RTX 4060 (8GB) with PyTorch 2.1.0 (CUDA 12.1), $\alpha$--$\beta$-CROWN (commit d4c79e3), and fixed seed 42.
...
\end{axis}
\end{tikzpicture}
\caption{Expert empirical results supporting the formal findings (AA at $\varepsilon = 8/255$, ExCRA at $\varepsilon = 2/255$; mean $\pm$ std over 5 runs). Because the perturbation budgets differ, absolute values are not directly comparable. The key observation is that within each domain, the empirical results preserve the same model ordering observed under certified robustness analysis. CIFAR-10 NRT is completely unverifiable (ExCRA $= 0\%$). Router results are omitted because RoCRA (routing invariance) and router AA (end-to-end classification) measure different properties.}
...
\node[font=\scriptsize, anchor=west] at (axis cs:94,{Monolithic (2 datasets)}) {\textbf{91.5}};
\node[font=\scriptsize, anchor=west] at (axis cs:102,{MetaMoE (2 datasets)}) {\textbf{99.5}};
\node[font=\scriptsize, anchor=west] at (axis cs:245,{Monolithic (+MNIST)}) {\textbf{242.5}};
\node[font=\scriptsize, anchor=west] at (axis cs:205,{MetaMoE (+MNIST)}) {\textbf{202.0}};
\end{axis}
\end{tikzpicture}
\caption{Training time: monolithic vs.\ \textsf{MetaMoE} for 2 datasets and incremental MNIST addition. Monolithic retrains from scratch (red); \textsf{MetaMoE} trains only the new expert (green) and fine-tunes the router (orange), yielding a 16.7\% speedup.}
\label{fig:training-time}
\end{figure}

For 2 datasets, costs are comparable (91.5 vs.\ 99.5 mins). The advantage emerges at scale: adding MNIST requires full retraining for the monolithic model (151 mins) but only independent expert training (72.5 mins) plus router fine-tuning (30 mins) for \textsf{MetaMoE}. Only the 30-min fine-tuning touches the existing system; expert training is fully parallelizable. This gap widens with each additional expert, and the same linear scaling applies to re-verification (${\sim}$2.4$\times$ at $N{=}2$, ${\sim}$7$\times$ at $N{=}7$).
...
The framework also depends on the router's ability to discriminate between expert domains. For visually dissimilar datasets (CIFAR-10 vs.\ MNIST), the router achieves 100\% gating accuracy and Theorem~\ref{thm:compositional} applies directly. For visually similar datasets (GTSRB vs.\ PTSD), gating accuracy drops to 93.17\%, violating router invariance at ${\sim}$7\% of inputs.
