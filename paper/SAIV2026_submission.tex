...
\begin{enumerate}
    ...
    \item \textbf{Empirical--Formal Correlation:} \BW{first introduction, I think using different terminology would be helpful. I think we should come up with a different phrase.} Does PGD adversarial accuracy predict certified robustness accuracy?
...
Two complementary metrics quantify robustness: \emph{adversarial accuracy} $\text{AA}(\varepsilon)$, the fraction of test samples correctly classified under a PGD attack with perturbation budget~$\varepsilon$, and \emph{certified robustness accuracy} $\text{CRA}(\varepsilon)$, the fraction for which a formal verifier proves robustness within the $\varepsilon$-ball. \BW{If the approach is formal I think the AA should remain at 100\%, i.e. it is robust and so nothing changes. I also don't understand how CRA would be a percentage, I think it should be be binary, either robust or not robust.} At a fixed~$\varepsilon$, soundness of the verifier guarantees $\text{CRA} \leq \text{AA}$, since every certified sample is truly robust while PGD may miss adversarial examples. In practice, AA is typically evaluated at the training~$\varepsilon$ (e.g., $8/255$) and CRA at a smaller verification~$\varepsilon$ (e.g., $2/255$), so their absolute values are not directly comparable. \BW{Why would we compare different epsilons?} We therefore focus on whether the \emph{ranking} of models by AA predicts their ranking by CRA. Prior work has established that adversarial training generally improves certified robustness~\cite{salman2019provably,balunovic2020adversarial}, and surveys confirm this trend across verification methods~\cite{li2023sok}. Building on this foundation, we investigate whether, within a given domain and architecture, higher AA monotonically associates with higher CRA, with robust training as a prerequisite for verifiability. Our experiments confirm this monotonic association and further reveal that non-robust training on complex domains yields complete verification failure (CRA $= 0\%$), a finding we believe to be novel. \BW{I think this section could do with some work, if AA and CRA are used in other works we should cite them and provide more background, for formal verification folks I think they could reject the paper off this section since it doesn't seem formal. One thing thats unclear to me is are these metrics for a single problem? Like one image is robust? Or is it the whole class of images? I think if we are taking about many tests of different images these metrics make more sense. }
