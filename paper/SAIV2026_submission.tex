...
\end{equation}
A positive gap ($\Delta > 0$) indicates that formal verification certifies more samples than empirical attacks can defeat, reflecting conservatism of the attack rather than looseness of the verifier. Note that CRA and AA may measure different properties: for example, router CRA verifies expert-selection invariance on individual samples, while router AA measures end-to-end classification accuracy under attack on the mixed test set, so large gaps between them do not indicate inconsistency. Prior work has established that adversarial training generally improves certified robustness~\cite{salman2019provably,balunovic2020adversarial}, and surveys confirm this trend across verification methods~\cite{li2023sok}. Building on this foundation, we formalize the relationship as a quantitative metric and hypothesize that within a given domain and architecture, higher AA monotonically associates with higher CRA, with robust training as a prerequisite for and predictive of verifiability. Our experiments validate this hypothesis and further reveal that the gap is domain-dependent ($\Delta = 9\%$ for MNIST-RT vs.\ $\Delta = 73\%$ for CIFAR-10-RT) and that non-robust training on complex domains yields complete verification failure (CRA $= 0\%$), a finding we believe to be novel.
