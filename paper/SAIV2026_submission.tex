...
\end{equation}
When CRA and AA measure the same property on the same inputs, CRA $\leq$ AA must hold because formal verification is sound (every certified sample is truly robust) while PGD is a heuristic that may miss adversarial examples. However, in our MoE setting CRA and AA sometimes measure different properties: for example, router CRA verifies expert-selection invariance on individual samples, while router AA measures end-to-end classification accuracy under attack on the mixed test set. In such cases $\Delta$ can be positive and direct comparison is not meaningful; we report it to characterize the relationship between empirical and formal metrics rather than as a consistency check. Prior work has established that adversarial training generally improves certified robustness~\cite{salman2019provably,balunovic2020adversarial}, and surveys confirm this trend across verification methods~\cite{li2023sok}. Building on this foundation, we investigate whether, within a given domain and architecture, higher AA monotonically associates with higher CRA, with robust training as a prerequisite for verifiability. Our experiments confirm this monotonic association and further reveal that the gap is domain-dependent ($\Delta = 9\%$ for MNIST-RT vs.\ $\Delta = 73\%$ for CIFAR-10-RT) and that non-robust training on complex domains yields complete verification failure (CRA $= 0\%$), a finding we believe to be novel.
