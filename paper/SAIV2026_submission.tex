...
Formal certification and empirical adversarial evaluation play different roles in this paper. \emph{Expert Certified Robustness Accuracy} $\text{ExCRA}(\varepsilon)$) is a \textbf{formal metric}: it is the primary decision criterion because it provides a provable guarantee that no perturbation within $B_\varepsilon(x)$ can change the prediction (Section~\ref{sec:threat-model}). \emph{Adversarial accuracy} $\text{AA}(\varepsilon)$ under PGD ~\cite{madry2018} is used only as supporting empirical evidence. At a fixed~$\varepsilon$, soundness guarantees $\text{ExCRA} \leq \text{AA}$ since every certified image is truly robust while PGD may miss adversarial examples. In practice, $\text{AA}$ is typically evaluated at the training budget (e.g., $8/255$), and $\text{ExCRA}$ at a smaller verification budget (e.g., $2/255$), so their absolute values are not directly comparable. We therefore do not interpret $\text{AA}$ as a substitute for $\text{ExCRA}$. Instead, we examine whether empirical experiments support the formal findings by preserving the same model ordering and qualitative trends observed under certification
...
Prior work has established that adversarial training generally improves certified robustness~\cite{salman2019provably,balunovic2020adversarial}, and surveys confirm this trend across verification methods~\cite{li2023sok}. Our experiments show that, within a fixed domain and architecture, stronger empirical robustness is consistent with stronger certified robustness and further reveal that non-robust training on complex domains yields complete verification failure (ExCRA $= 0\%$), a finding we believe to be novel.

\section{Methodology}

Given an RGB input image $x \in \mathbb{R}^{C \times H \times W}$ and $N$ domain-specific datasets each containing $C_i$ classes, we independently train $N$ expert networks $E_i: \mathbb{R}^{C \times H \times W} \to \mathbb{R}^{C_i}$ (one per dataset, frozen after training) and a single router $\mathcal{G}: \mathbb{R}^{C \times H \times W} \to \mathbb{R}^N$ that outputs one routing logit per expert. We evaluate on 4 datasets: CIFAR-10~\cite{cifar10} (natural images, 10 classes), MNIST~\cite{mnist1998} (handwritten digits, 10 classes), GTSRB~\cite{gtsrb} (German traffic signs, 43 classes), and PTSD~\cite{ptsd} (Persian traffic signs, 43 classes). The first two form a visually dissimilar pair; the latter two share visual structure, testing the router under harder conditions. The framework extends to additional traffic sign datasets (BTSD, ETSD, TSRD) for lifelong learning (Appendix~\ref{app:notation}).

Each expert $E_i$ is denoted $E_{i\_\text{NRT}}$ (non-robustly trained) or $E_{i\_\text{RT}}$ (robustly trained), where the index $i$ corresponds to: CIFAR-10 ($E_0$), MNIST ($E_1$), GTSRB ($E_2$), and PTSD ($E_3$).

\subsection{Verification-Aware CNN Architecture}

Both experts and router use a compact convolutional architecture (VerifiableCNN) designed for compatibility with $\alpha$--$\beta$-CROWN's bound propagation. Verification cost grows super-linearly with network size and nonlinearity count~\cite{wang2021}, and VNN-COMP benchmarks~\cite{kaulen2025} show that current complete verifiers scale to CNNs of ${\sim}$100K parameters within practical timeouts. We therefore target this regime: 4 convolutional layers with gradual channel growth ($20 \to 28 \to 40 \to 56$), $2{\times}2$ average pooling in blocks 1--3, and a 2-layer fully connected head (detailed specifications in Appendix~\ref{app:architecture}). Three design choices make the network amenable to LP-based verification: average pooling replaces max pooling to preserve linearity (each max node introduces a branching point for the LP solver), increased channel capacity compensates for narrow widths, and static shapes minimize ONNX operators. Experts include BatchNorm (${\sim}$96K params for 43 classes), which we fold into convolutional weights during ONNX export (Appendix~\ref{app:onnx}) to eliminate BatchNorm operators from the verification graph. The router omits BatchNorm entirely for deterministic verification behavior (${\sim}$38K params). The architecture is evaluated on both formal metrics (ExCRA and RoCRA via $\alpha$--$\beta$-CROWN) and empirical metrics (clean accuracy and adversarial accuracy under PGD), with ExCRA as the primary decision criterion.

We train experts independently using Adam~\cite{kingma2015adam} (learning rate $10^{-3}$, batch size 128, 200 epochs with early stopping) with label-smoothing cross-entropy~\cite{szegedy2016labelsmoothing} (smoothing $= 0.1$) and standard augmentation (random flips and crops) under two paradigms: Non-Robust Training (NRT) with standard cross-entropy, and Robust Training (RT) with PGD adversarial training~\cite{madry2018,zhang2019trades} ($\varepsilon = 8/255$, 7 steps, step size $\alpha = 2/255$, $L_\infty$ norm), deliberately trading clean accuracy for improved robustness against bounded perturbations.
...
Once all experts are frozen, the router is trained on meta-class labels $m \in \{0, \ldots, N{-}1\}$ using Adam (learning rate $10^{-3}$, weight decay $5 \times 10^{-4}$, batch size 128, 50 epochs with early stopping). Under Non-Robust Training (NRT), the objective is simply the cross-entropy between router predictions and the true meta-class:
\begin{equation}
...
Standard $\alpha$--$\beta$-CROWN verifies monolithic networks. Verifying our \textsf{MetaMoE} system (router + 2~experts $\approx$~231K parameters) monolithically is not merely expensive -- it is \emph{structurally infeasible} with current tools, due to two independent structural barriers. First, ONNX export is trace-based~\cite{pytorchonnx}: the exporter records a single execution path, hard-coding the router's expert selection for the dummy input into a static graph~\cite{pytorchcontrolflow}. The ONNX IR is fundamentally a static, side-effect-free computation graph~\cite{onnxir}; its \texttt{If} operator requires identically shaped branch outputs~\cite{pytorchcontrolflow}, which is incompatible with experts having different class space dimensions. Second, all current verifiers operate on fixed computational graphs. Bound propagation (CROWN~\cite{xu2020abcrown}, $\alpha$--$\beta$-CROWN~\cite{wang2021}) is undefined when graph topology varies per input; SMT-based (Marabou~\cite{katz2022marabou}) and abstract-interpretation (ERAN~\cite{singh2019eran}) verifiers likewise require fixed topologies. No VNN-COMP benchmark (2021--2025) has included models with input-dependent routing~\cite{kaulen2025}. Our compositional decomposition sidesteps both barriers by reducing the problem to standard feedforward CNN verification, yielding $O(1)$ per-component cost and compatibility with all existing monolithic verifiers. This motivates a \textbf{compositional verification strategy} that decomposes the~problem:
...
All experiments use VerifiableCNN experts (${\sim}$94K parameters, 10 classes; ${\sim}$96K for 43 classes) and a VerifiableCNN router without BatchNorm (${\sim}$38K parameters), trained under NRT or RT (Section~4). All results are averaged over 5 independent runs. Training and verification ran on an NVIDIA GeForce RTX 4060 (8GB) with PyTorch 2.1.0 (CUDA 12.1), $\alpha$--$\beta$-CROWN (commit d4c79e3), and fixed random seed 42.

\textbf{Test sets and sample sizes.} For formal verification, we sample a fixed subset of correctly classified images (20--40 per configuration) because per-image verification is expensive (${\sim}$11s per router sample, up to 300s per expert sample). For empirical PGD attacks, we evaluate on the full test set of each dataset (10,000 for CIFAR-10/MNIST; 12,630 for GTSRB; 2,421 for PTSD).

\textbf{Experiments.} We run four groups: (1)~router formal verification: 40 samples at $\varepsilon \in \{2/255, 4/255, 8/255\}$ under NRT and RT; (2)~expert formal verification: 20 samples per configuration across 4 expert variants and 3~$\varepsilon$ budgets; (3)~empirical adversarial robustness via PGD ($\varepsilon = 8/255$) on full test sets for individual experts and all 8 MoE configurations (2 router $\times$ 4 expert combinations, 40 total runs); and (4)~empirical--formal comparison of AA and ExCRA orderings.

\textbf{Per-image procedure.} Every metric is computed from a per-image binary outcome aggregated over the test set. For \emph{Clean Accuracy} and \emph{Gating Accuracy}, each image is classified once and scored correct/incorrect. For \emph{Adversarial Accuracy} (AA), PGD generates a worst-case perturbation within $\|\delta\|_\infty \leq \varepsilon$; the image scores 1 if the model still predicts correctly. Because PGD is heuristic, AA upper-bounds true robustness. For \emph{ExCRA} and \emph{RoCRA} (defined in Section~\ref{sec:threat-model}), $\alpha$--$\beta$-CROWN attempts a formal proof over the entire $\varepsilon$-ball; the image scores 1 only on a completed proof (timeouts and counterexamples both score~0), making these metrics lower bounds on true robustness.
...
RoCRA ($\approx 100\%$) substantially exceeds end-to-end AA ($\approx 35\%$). This is expected: RoCRA verifies only the property $\arg\max_i \mathcal{G}_i(x') = m^*$ (correct expert selection) on individual samples, while AA measures end-to-end classification on the mixed test set where expert misclassifications dominate. The router achieves 100\% gating accuracy under PGD, but the selected expert may misclassify, reducing end-to-end AA. At a fixed $\varepsilon$, formal verification is sound so $\text{ExCRA} \leq \text{AA}$ always holds; the apparent inversion here arises because RoCRA and AA use different perturbation budgets ($\varepsilon = 2/255$ vs.\ $8/255$) and measure different properties.
...
GTSRB+PTSD (visually similar traffic sign datasets) achieves only 93.17\% gating accuracy and 29.47\% AA, compared to 100\% and 41.93\% for CIFAR10+MNIST (Both RT). The 12.12\% AA gap demonstrates that routing difficulty directly propagates to system robustness. Clean accuracy standard deviations range from 0.04\% to 0.08\%, indicating highly reproducible training outcomes.

\subsection{Empirical Results Supporting the Formal Findings}
\label{sec:eval-correlation}

Figure~\ref{fig:gap-analysis} compares expert AA (PGD, $\varepsilon = 8/255$) with ExCRA ($\alpha$--$\beta$-CROWN, $\varepsilon = 2/255$). Because the perturbation budgets differ, their absolute values are not directly comparable. Our goal is therefore not to match AA and ExCRA numerically, but to test whether the empirical results support the formal findings by preserving the same ordering across models. Within each domain, this is exactly what we observe: MNIST NRT$\to$RT increases both AA ($46\% \to 91\%$) and ExCRA ($75\% \to 100\%$), while CIFAR-10 NRT$\to$RT moves from complete verification failure (ExCRA $= 0\%$) to ExCRA $= 90\%$. Routers are excluded from this comparison because RoCRA verifies routing invariance, whereas router AA measures end-to-end classification; the two metrics do not address the same property.
...
The practical implication is not that empirical attacks replace certification. Rather, the empirical results support the formal findings by showing that the same robustness trends appear in both analyses. In our study, the models that are stronger empirically within each domain are also the ones that achieve stronger certified robustness, while non-robustly trained models on complex domains fail to certify at all.

\subsection{Scalability}
\label{sec:eval-scalability}

\textbf{Training time.}
We compared \textsf{MetaMoE} against monolithic training across two scenarios: initial 2-dataset training (GTSRB + CIFAR-10) and incremental addition of a third dataset (MNIST). Figure~\ref{fig:training-time} shows the full breakdown.
...
For 2 datasets, costs are comparable (91.5 vs.\ 99.5 mins). The compositional advantage emerges at scale: adding MNIST requires full retraining for the monolithic model (151 mins) but only an independent expert training (72.5 mins) plus router fine-tuning (30 mins) for \textsf{MetaMoE}. Of this incremental cost, only the 30-min router fine-tuning touches the existing system; expert training is fully parallelizable. This gap widens with each additional expert, since per-expert cost is fixed while monolithic retraining grows with dataset count. The same linear scaling applies to re-verification: our $N{=}2$ setting yields a ${\sim}$2.4$\times$ cost reduction, growing to ${\sim}$7$\times$ at $N{=}7$.
