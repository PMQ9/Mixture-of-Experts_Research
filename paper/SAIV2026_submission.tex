...
\begin{axis}[
    ...
    xmin=0, xmax=290,
    legend style={at={(0.5,-0.42)}, anchor=north, font=\scriptsize, legend columns=5},
    grid=major,
    grid style={dashed, gray!20},
...
% Combined/joint training (gray) -- monolithic only
...
% Individual expert training (blue) -- MoE only
...
% Router training/fine-tuning (orange) -- MoE only
...
\legend{Joint training, Expert(s), New expert, Router, Full retrain}
...
% Monolithic (2 datasets): gray 91.5
...
% MetaMoE (+MNIST): blue 87.5 | green 72.5 | orange 42
\node[font=\tiny\bfseries] at (axis cs:43.75,{MetaMoE (+MNIST)}) {87.5};
\node[font=\tiny\bfseries] at (axis cs:123.75,{MetaMoE (+MNIST)}) {72.5};
\node[font=\tiny\bfseries] at (axis cs:181,{MetaMoE (+MNIST)}) {42};
\end{axis}
\end{tikzpicture}
\caption{Training time: monolithic vs.\ \textsf{MetaMoE} for initial 2-dataset training (top) and incremental MNIST addition (bottom). Top pair: costs are comparable (91.5 vs.\ 99.5\,min). Bottom pair: monolithic retrains all three datasets from scratch (gray\,+\,red\,=\,242.5\,min), while \textsf{MetaMoE} reuses frozen experts (blue), trains only the new MNIST expert (green), and fine-tunes the router (orange), saving 16.7\% overall. The incremental cost alone drops from 151\,min (monolithic retrain) to 102.5\,min (expert\,+\,router), a 32\% reduction.}
\label{fig:training-time}
\end{figure}

For 2 datasets (top pair), costs are comparable (91.5 vs.\ 99.5 mins) because both approaches must train on the same data. The advantage emerges when scaling: the bottom pair shows that adding MNIST forces the monolithic model to retrain all three datasets from scratch (gray initial~+~red retrain~=~242.5 mins), whereas \textsf{MetaMoE} reuses the frozen 2-dataset experts (blue, already paid), trains only the new MNIST expert independently (green, 72.5 mins), and fine-tunes the router on the combined validation set (orange, 30 mins). The incremental cost is therefore 151 mins (monolithic) vs.\ 102.5 mins (\textsf{MetaMoE}), a 32\% reduction. Crucially, only the 30-min router fine-tuning step touches the existing system; the 72.5-min expert training is fully independent and parallelizable. This gap widens with each additional expert, and the same linear scaling applies to re-verification (${\sim}$2.4$\times$ at $N{=}2$, ${\sim}$7$\times$ at $N{=}7$).
