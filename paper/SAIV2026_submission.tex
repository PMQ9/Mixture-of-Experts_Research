...
\label{eq:first}
    ...
    \end{cases}
\end{equation}
where $y_{s_j}[c - o_{s_j}]$ is expert~$s_j$'s logit at local class index $c - o_{s_j}$, and the per-index renormalization makes contributions a convex combination of expert logits when multiple activated experts share that class. In the disjoint-class hard-routing regime verified here, $|\mathcal{A}(c)| \leq 1$ and \eqref{eq:first} collapses to $\hat{y}_c = y_{s_{j^*}}[c - o_{s_{j^*}}]$; the general form is stated for the soft and top-$k$ extensions of Section~\ref{sec:limitations}.
...
Theorem~\ref{thm:compositional} establishes a biconditional only for hard routing ($k{=}1$) with disjoint class spaces; soft or top-$k$ routing requires invariance of the full top-$k$ set and routing-weight stability, and overlapping class spaces weaken the result to a one-sided implication (Section~\ref{sec:compositional}). The framework also depends on router discrimination: CIFAR-10 vs.\ MNIST gives 100\% gating accuracy, while visually similar GTSRB vs.\ PTSD drops to 93.17\%, violating invariance at ${\sim}$7\% of inputs.

Our experiments use small verification-compatible CNNs on 20--40 samples per configuration (${\sim}$11s/router, up to 300s/expert); larger experts or sample budgets remain bounded by current tool capacity. We consider only $\ell_\infty$ perturbations; $\ell_2$ and patch attacks need separate specifications.

\textbf{Scope of the certified guarantee.} Theorem~\ref{thm:compositional} certifies each tested cube $B_\varepsilon(x)$ independently, not the input distribution -- a \emph{cube-wise} guarantee (Casadio et al.'s constraint security~\cite{casadio2022}) shared by all current complete verifiers~\cite{kaulen2025} and silent about unseen images. PGD-AA~\cite{madry2018} provides the empirical complement.
...
The compositional perspective introduced here provides a foundation for building verifiable modular AI in safety-critical applications.
