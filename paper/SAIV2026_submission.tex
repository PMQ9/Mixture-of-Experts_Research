...
\end{axis}
\end{tikzpicture}
\caption{Training time breakdown: monolithic vs.\ \textsf{MetaMoE} for 2 datasets (GTSRB + CIFAR-10) and incremental addition of MNIST. Monolithic training requires full retraining from scratch (red), while \textsf{MetaMoE} only trains the new expert (green) and fine-tunes the router (orange), yielding a 16.7\% speedup that grows with each additional expert.}
