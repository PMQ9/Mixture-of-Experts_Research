...
A striking observation is that the router achieves empirical adversarial accuracy (AA) of approximately 35\% while formal verification certifies 100\% robustness at tight ($\varepsilon = 2/255$) and moderate ($\varepsilon = 4/255$) perturbations, and 60-100\% at large perturbations ($\varepsilon = 8/255$). This apparent contradiction reveals fundamental differences between empirical attacks and formal worst-case bounds, explained by geometric separation between datasets.

The key insight is that MNIST (grayscale 10-class digits) and CIFAR-10 (natural color images with 10 classes) occupy fundamentally disjoint regions of the input space at the pixel level, creating a large decision boundary margin. While PGD perturbations with $\varepsilon = 8/255$ are sufficient to cause routing errors in empirical attacks via gradient-guided perturbation sequences (reducing routing accuracy from 100\% clean to roughly 35\% adversarial), the formal verification bounds account for \textit{all possible} perturbations within the $\varepsilon$-ball. The $\alpha$--$\beta$-CROWN analysis confirms that the router's learned decision regions remain well-separated: the separation between MNIST-space and CIFAR-10-space far exceeds $\varepsilon = 8/255$ pixels in Euclidean distance, so even worst-case perturbations cannot cross the decision boundary.

This is not a contradiction but rather a fundamental property of formal verification: CRA measures worst-case geometric robustness (whether the decision boundary can be crossed by any perturbation in the ball), while AA measures empirical vulnerability to specific heuristic attacks (PGD's gradient-based search). Both are scientifically valid but measure different aspects: CRA provides sufficient conditions for robustness via formal bounds; AA provides necessary conditions by revealing practical vulnerabilities. The high CRA and moderate AA for routers reflects the geometric dissimilarity of datasets: formal verification proves decision boundaries are separated by margin $\gg \varepsilon$, while PGD finds perturbation sequences that exploit the manifold geometry within the $\varepsilon$-ball. These observations support the compositional guarantees: verified routers (which route to the correct expert with CRA $\approx 100\%$ at practical threat models) combined with verified experts yield verified systems.
...
RT models consistently achieve higher adversarial accuracy (AA) and certified robustness accuracy (CRA) than NRT: MNIST-RT verified 100\% vs 75\% for NRT at $\varepsilon = 2/255$, CIFAR-10-RT 90\% vs 0\%. The certification gap $\Delta = \text{CRA} - \text{AA}$ quantifies verification conservatism: experts show $\Delta = 9$--$73\%$ depending on dataset complexity and perturbation budget, while routers exhibit significantly different behavior. Router empirical-formal alignment reveals a critical insight: routers achieve empirical adversarial accuracy (AA) of only 35\% but formal certified robustness (CRA) of 100\% at practical threat models ($\varepsilon \leq 4/255$), demonstrating that geometric dataset separation creates formal robustness that exceeds empirical attack success. This large $\Delta \approx 65\%$ for routers (unlike experts) reflects decision regions separated by margin $\gg \varepsilon$, enabling perfect formal verification despite moderate empirical vulnerability. Adversarial router training improves both empirical AA and extends formal verification to large perturbations ($\varepsilon = 8/255$, improving from 60\% to 100\% success), demonstrating that formal robustness benefits from decision boundary smoothing even when empirical AA remains moderate.
