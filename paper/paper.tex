...
Heterogeneous Mixture-of-Experts (MoE) systems composed of domain-specific experts and a routing network offer scalable solutions for safety-critical applications. However, understanding how robustness propagates across modular components under adversarial perturbations remains an open challenge. This work bridges empirical adversarial robustness and formal verification to establish both practical resilience and provable guarantees for compositional MoE systems.

We introduce MetaMoE, a heterogeneous MoE architecture where experts and routers are jointly optimized for adversarial robustness and $\alpha$-$\beta$-CROWN formal verifiability. We demonstrate that robustness exhibits compositional structure: expert training dominates system robustness, while router training shows minimal robustness impact when datasets are visually dissimilar. Crucially, we validate a strong empirical-formal correlation: models achieving higher adversarial accuracy under PGD attacks consistently attain higher certified robustness accuracy under formal verification. We further establish that perfect routing under perturbation enables compositional verification: when routers maintain consistent expert selection within adversarial neighborhoods, verified experts compose to verified systems without full-system re-verification.

These findings establish a unified framework linking empirical robustness and formal verification in heterogeneous MoE systems. Verification-aware architecture design enables both high empirical adversarial accuracy and formal certification, supporting scalable deployment in safety-critical applications where new experts can be added and verified incrementally without retraining the entire system.
...
\textbf{Understanding the Router AA vs CRA Discrepancy:}

A striking observation is that the router achieves empirical adversarial accuracy (AA) of approximately 35\% while formal verification certifies near-complete robustness (CRA $\approx$ 97.5--100\%). This apparent contradiction reveals fundamental differences between empirical attacks and formal worst-case bounds.

The explanation lies in geometric separation: MNIST (grayscale 10-class digits) and CIFAR-10 (natural color images with 10 classes) occupy fundamentally disjoint regions of the input space at the pixel level. While PGD perturbations with $\varepsilon = 8/255$ are sufficient to cause routing errors in empirical attacks (reducing routing accuracy from 100\% clean to roughly 35\% adversarial), formal verification operates at a different scale. The $\alpha$--$\beta$-CROWN bounds, which conservatively account for all possible perturbations within the $\varepsilon$-ball, still confirm that the router's learned decision regions remain well-separated. This is because the separation between MNIST-space and CIFAR-10-space far exceeds $\varepsilon = 8/255$ pixels; PGD does not reach this geometric boundary despite degrading empirical accuracy. In other words, formal verification proves routing robustness at a coarser grain (whether domains are separable within the perturbation ball), while PGD measures empirical susceptibility to specific local perturbations. Both are valid but measure different aspects: CRA measures worst-case formal guarantees; AA measures vulnerability to practical (heuristic) attacks. The high CRA and moderate AA for routers reflects domain dissimilarity that enables formal robustness while remaining vulnerable to adversarial perturbation sequences in feature space.
...
\textbf{Clarification: Why CRA $>$ AA (Formal Verification vs Empirical Attacks):}

A counterintuitive observation in Table 4 is that CIFAR-10-RT achieves CRA = 90\% while AA = 17.24\%, and MNIST-RT achieves CRA = 100\% while AA = 90.77\%. This apparent contradiction is resolved by understanding the fundamental difference between empirical attacks and formal worst-case bounds:

\begin{itemize}
    \item \textbf{Certified Robustness Accuracy (CRA):} Measures the fraction of samples \textit{provably robust} within the $\varepsilon$-ball according to $\alpha$--$\beta$-CROWN. A sample is certified if \textit{all possible perturbations} within the ball cannot flip the classification.

    \item \textbf{Adversarial Accuracy (AA):} Measures empirical robustness against \textit{specific attack sequences} (here, PGD with 7 iterations). PGD is a heuristic that may not find the worst-case adversarial example.

    \item \textbf{The Key Insight:} PGD attacks are incomplete; they search over a subset of the perturbation space. For CIFAR-10-RT with AA = 17.24\%, PGD successfully corrupts roughly 83\% of predictions. However, $\alpha$--$\beta$-CROWN's formal bounds certify that 90\% of samples remain robust to \textit{all} perturbations within $\varepsilon = 2/255$. This occurs when:
    \begin{enumerate}
        \item The model is locally vulnerable to specific PGD trajectories (lower AA)
        \item But the decision boundary remains globally separated from the adversarial perturbation ball (higher CRA)
        \item Formal verification pessimistically bounds all possible adversarial directions, while PGD explores only certain gradient-guided directions
    \end{enumerate}

    \item \textbf{For MNIST-RT:} CRA and AA are nearly aligned (100\% vs 90.77\%), reflecting a simpler domain where PGD's heuristic search closely approximates worst-case behavior, and decision boundaries are well-separated.
\end{itemize}

In summary: CRA $\geq$ AA is not a paradox but a fundamental property of formal verification—formal methods provide \textbf{sufficient conditions} for robustness that may be more conservative than necessary (high CRA, moderate AA), while empirical attacks provide \textbf{necessary conditions} that reveal specific vulnerabilities (lower AA). Both are scientifically valid; they measure different aspects of robustness.
